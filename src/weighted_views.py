...
def qm7filetowvmats(alist, speciesmap, carbonbased= False, not_hydrogen= False, heaviest_origin= True, viewlength=None):
    """
...
def load_qm7_data(train_mollist, speciesmap, setNatoms=None, setNviews=None, carbonbased= False, not_hydrogen= False, heaviest_origin= False,verbose=0):
    species = set()
    Natomslist = [len(mol) for mol in train_mollist]
    for mol in train_mollist:
    ...
    if verbose:
        print(f"{len(train_mollist)} molecules will be processed.")
